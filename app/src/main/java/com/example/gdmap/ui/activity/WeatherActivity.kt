...
class WeatherActivity : BaseActivity(), WeatherSearch.OnWeatherSearchListener {
    ...
    override fun onWeatherForecastSearched(
        localWeatherPredictResult: LocalWeatherForecastResult?,
        code: Int
    ) {
        if (code == 1000) {
            if (localWeatherPredictResult != null && localWeatherPredictResult.forecastResult != null) {
                ...
                tv_activity_weather_predicttmp.text =
                    weatherPrediction.weatherForecast[0].dayTemp + " 度"
                tv_activity_weather_predicttime2.text = weatherPrediction.weatherForecast[1].date
                tv_activity_weather_predicttmp2.text =
                    weatherPrediction.weatherForecast[1].dayTemp + " 度"
                tv_activity_weather_predicttime3.text = weatherPrediction.weatherForecast[2].date
                tv_activity_weather_predicttmp3.text =
                    weatherPrediction.weatherForecast[2].dayTemp + " 度"
            } else {
                Toast.toast("没有查询到天气情况")
            }
        }
